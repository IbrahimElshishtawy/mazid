...
class _UsersManagementScreenState extends State<UsersManagementScreen> {
  ...
  Widget build(BuildContext context) {
    ...
    final selectedUser = widget.users[_selectedUserIndex.clamp(0, widget.users.length - 1)];
        ...
        floatingActionButton: FloatingActionButton.extended(
          onPressed: () {
            ScaffoldMessenger.of(context).showSnackBar(
              const SnackBar(content: Text('تم تجهيز قسم إنشاء المستخدم، ويمكن تحويله إلى نموذج حفظ فعلي لاحقًا.')),
            );
          },
              ...
              child: Column(
                ...
                children: [
                  ...
                  _AdminModules(
                    ...
                    onUserChanged: (index) => setState(() => _selectedUserIndex = index),
                  ),
                  const SizedBox(height: 18),
                  SectionCard(
    ...
    return Column(
      children: [
        SectionCard(
          ...
          child: Column(
            children: [
              const _FakeField(label: 'الاسم الكامل', value: 'أدخل اسم المستخدم الجديد'),
              const SizedBox(height: 12),
              const _FakeField(label: 'البريد الإلكتروني', value: 'name@factory.com'),
              const SizedBox(height: 12),
              const _FakeField(label: 'الدور الوظيفي', value: 'Manager / Supervisor / Worker'),
              const SizedBox(height: 14),
              Align(
                ...
                child: FilledButton.icon(
                  onPressed: () {
                    ScaffoldMessenger.of(context).showSnackBar(
                      const SnackBar(content: Text('يمكن توصيل إنشاء المستخدم بقاعدة البيانات في الخطوة التالية.')),
                    );
                  },
                  icon: const Icon(Icons.person_add_alt_1_rounded),
  ...
  Widget build(BuildContext context) {
    ...
    return Column(
      children: [
        ...
        Row(
          children: [
            Expanded(child: _ActionCard(title: 'الصلاحيات الحالية', subtitle: permissionLevel, icon: Icons.shield_rounded, color: const Color(0xFF0F766E))),
            const SizedBox(width: 12),
            Expanded(child: _ActionCard(title: 'حالة الحساب', subtitle: user.status, icon: Icons.monitor_heart_rounded, color: const Color(0xFF2563EB))),
          ],
        ),
        const SizedBox(height: 14),
        SectionCard(
          ...
          child: Wrap(
            ...
            children: [
              OutlinedButton.icon(
                onPressed: () => _showSnack(context, 'تم منح صلاحية إضافية إلى ${user.name}'),
                icon: const Icon(Icons.add_moderator_outlined),
                label: const Text('منح صلاحية'),
              ),
              OutlinedButton.icon(
                onPressed: () => _showSnack(context, 'تم تقليل صلاحيات ${user.name}'),
                icon: const Icon(Icons.remove_moderator_outlined),
                label: const Text('تقليل الصلاحية'),
              ),
              OutlinedButton.icon(
                onPressed: () => _showSnack(context, 'تم تعليق حساب ${user.name} إداريًا'),
        ...
        SectionCard(
          ...
          child: Column(
            children: [
              const _FakeField(label: 'عنوان المهمة', value: 'مثال: مراجعة خط الإنتاج B'),
              const SizedBox(height: 12),
              const _FakeField(label: 'وصف المهمة', value: 'تفاصيل سريعة عن المطلوب تنفيذه'),
              const SizedBox(height: 12),
              ...
              Align(
                alignment: Alignment.centerRight,
                child: FilledButton.icon(
                  onPressed: () => _showSnack(context, 'تم تجهيز إرسال مهمة جديدة إلى ${user.name}'),
...
class _ActionCard extends StatelessWidget {
  const _ActionCard({required this.title, required this.subtitle, required this.icon, required this.color});
          ...
          Expanded(
            child: Column(
              ...
              children: [
                ...
                const SizedBox(height: 4),
                Text(subtitle, style: TextStyle(color: color, fontWeight: FontWeight.w900)),
              ],
            ),
          ),
        ],
...
class _UserManagementCard extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    final statusColor = user.status == 'نشط' || user.status == 'في الوردية' ? const Color(0xFF16A34A) : const Color(0xFF64748B);
